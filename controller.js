...
async function createAccount(req, res) {
    ...
    try {
        ...
        res.status(500).send({ status: false, message: 'Create Account Failed', error : error.message });
    }
}
...
async function getBalance(req, res) {
    ...
    try {
        ...
        res.status(500).send({ status: false, message: 'Get BNB Balance Failed', error : error.message });
    }
}
...
async function getTokenBalance(req, res) {
    ...
    try {
        // contract instance
        const contract = new web3.eth.Contract(abi, req.body.contract);
        ...
        res.status(500).send({ status: false, message: 'Get Token Balance Failed', error : error.message });
    }
}
...
async function transfer(req, res) {
    ...
    try {
        ...
        res.status(500).send({ status: false, message: 'Transfer Failed', error : error.message });
    }
}
...
async function transferToken(req, res) {
    ...
    try {
        // contract instance
        const contract = await new web3.eth.Contract(abi, req.body.contract);
        const decimals = await contract.methods.decimals().call();
        // transfer event abi
